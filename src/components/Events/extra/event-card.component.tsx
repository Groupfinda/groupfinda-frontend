    ...
    >
      <Layout style={styles.contentContainer}>
        <Layout level="1" style={styles.dateContainer}>
          <Icon width={16} height={16} fill="black" name="calendar-outline" />
          <Text>{new Date(event.dateOfEvent).getDate()}</Text>
          <Text>{months[new Date(event.dateOfEvent).getMonth()]}</Text>
          <Text>
            {padZero(new Date(event.dateOfEvent).getHours())}:
            {padZero(new Date(event.dateOfEvent).getMinutes())}
          </Text>
        </Layout>
        <Divider />
        <Layout level="2" style={styles.descriptionContainer}>
          ...
          <Text style={{ color: "grey" }} category="s1">
            {event.description}
          </Text>
          <Layout style={styles.footerText}>
            <Icon width={16} height={16} fill="black" name="people-outline" />
            <Text> Group Size: </Text>
            <Text status="primary">{event.groupSize}</Text>
          </Layout>
          <Layout style={styles.footerText}>
            <Icon width={16} height={16} fill="black" name="clock-outline" />
            <Text> Sign-ups Close: </Text>
            <Text status="warning">
              {new Date(event.dateLastRegister).getDate()}{" "}
              {months[new Date(event.dateLastRegister).getMonth()]}{" "}
              {new Date(event.dateLastRegister).getFullYear()}
            </Text>
          </Layout>
        </Layout>
      </Layout>
